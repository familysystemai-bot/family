...
def _branch_location_json(branch_name: str, user_message: str = ""):
    ...
    """
    had_pending = chat_ctx.peek_pending_branch_kind_source() is not None
    kind_src = chat_ctx.merged_message_with_pending_branch_kind(user_message or "")
    try:
        cs = _cs()
        get_db = cs.get_db
        bid = get_db().get_branch_id_by_city_name(branch_name)
        ...
        kind = enhanced_location_reply_kind(kind_src)
